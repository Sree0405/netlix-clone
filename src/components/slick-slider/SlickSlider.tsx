      ...
      {data.results.length > 0 && (
        <>
          <Stack
            ...
            direction="row"
            alignItems="center"
            sx={{ mb: 2,mt:4, pl: { xs: "30px", sm: "60px" } }}
          >
            ...
            >
              {(() => {
                const suffix = mediaType === MEDIA_TYPE.Tv ? "TV Shows" : "Movies";
                const title = genre.name.toLowerCase().includes("movie") ||
                  genre.name.toLowerCase().includes("tv") ||
                  genre.name.toLowerCase().includes("show")
                  ? genre.name : `${genre.name} ${suffix}`;
                return title;
              })()}
              <MotionContainer
                open={showExplore}
